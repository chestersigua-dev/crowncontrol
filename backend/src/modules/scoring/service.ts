import { ScoringRepository } from './repository';
import { SubmitScoreDTO } from './dto';
import { broadcastScoreUpdate } from '../../config/socket';

export class ScoringService {
  private repository = new ScoringRepository();

  async submitScore(judgeId: string, data: SubmitScoreDTO) {
  ...
  async getScoresForSection(sectionId: string) {
    return this.repository.getScoresBySection(sectionId);
  }

  async getJudgeScore(judgeId: string, candidateId: string, sectionId: string) {
