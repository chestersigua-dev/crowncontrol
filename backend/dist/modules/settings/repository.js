...
class SettingsRepository {
    async getAllSections() {
        return database_1.prisma.pageantSection.findMany({
            include: { criteria: true },
            orderBy: { name: 'asc' },
        });
    }
    async getSectionById(id) {
        return database_1.prisma.pageantSection.findUnique({
            where: { id },
            include: { criteria: true },
        });
    }
    async getActiveSection() {
        return database_1.prisma.pageantSection.findFirst({
            where: { isActive: true },
            include: { criteria: true }
        });
    }
    async createSection(data) {
        return database_1.prisma.pageantSection.create({
            data,
        });
    }
    async updateSection(id, data) {
        return database_1.prisma.pageantSection.update({
            where: { id },
            data,
        });
    }
    async deleteSection(id) {
        return database_1.prisma.pageantSection.delete({
            where: { id },
        });
    }
    ...
    async createCriteria(data) {
        return database_1.prisma.criteria.create({
            data,
        });
    }
    async deleteCriteria(id) {
        return database_1.prisma.criteria.delete({
            where: { id },
        });
    }
    async getSetting(key) {
        return database_1.prisma.systemSetting.findUnique({
            where: { key }
        });
    }
    async upsertSetting(key, value) {
        return database_1.prisma.systemSetting.upsert({
            where: { key },
            update: { value },
            create: { key, value }
        });
